...
public:
    explicit ModelInstance(std::span<const pmx::Bone> bones);
...
class Scene final {
public:
    explicit Scene(float maximum_delta_seconds);

    ModelInstance& create_instance(std::span<const pmx::Bone> bones);
