...
int main() {
    ...
    std::vector<libmmd::pmx::Bone> layered_bones(2);
    layered_bones[0].name = "root";
    layered_bones[0].parent_index = -1;
    layered_bones[1].name = "arm";
    layered_bones[1].parent_index = -1;
    libmmd::MotionClip walking(layered_bones, source);
    libmmd::vmd::Motion arm_source{};
    auto arm_frame = frame(0, 8.0f);
    arm_frame.name.fill(std::byte{0});
    name(arm_frame.name, "arm");
    arm_source.bone_keyframes.push_back(arm_frame);
    arm_frame.frame = 30;
    arm_source.bone_keyframes.push_back(arm_frame);
    libmmd::MotionClip arm_motion(layered_bones, arm_source);
    libmmd::AnimationController layered(layered_bones);
    assert(layered.play(walking, true, 0.0f));
    assert(layered.play_overlay(arm_motion, true, 1.0f));
    assert(layered.update(0.25f));
    assert(std::abs(layered.pose().skinning_matrices()[12] - 0.5f) < 0.001f);
    assert(std::abs(layered.pose().skinning_matrices()[28] - 1.25f) < 0.001f);
    assert(layered.update(0.25f));
    assert(std::abs(layered.pose().skinning_matrices()[12] - 1.0f) < 0.001f);
    assert(std::abs(layered.pose().skinning_matrices()[28] - 4.0f) < 0.001f);
    assert(layered.stop_overlay(0.5f));
    assert(layered.update(0.25f));
    assert(std::abs(layered.pose().skinning_matrices()[12] - 1.5f) < 0.001f);
    assert(std::abs(layered.pose().skinning_matrices()[28] - 2.0f) < 0.001f);
    assert(layered.update(0.25f));
    assert(std::abs(layered.pose().skinning_matrices()[28]) < 0.001f);
    assert(layered.play_overlay(arm_motion, true, 0.0f));
    assert(layered.play(walking, true, 0.0f));
    assert(std::abs(layered.pose().skinning_matrices()[28] - 8.0f) < 0.001f);
    assert(layered.stop_overlay(0.0f));
    assert(std::abs(layered.pose().skinning_matrices()[28]) < 0.001f);
    return 0;
}
